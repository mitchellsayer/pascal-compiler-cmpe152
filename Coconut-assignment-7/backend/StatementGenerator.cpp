...
void StatementGenerator::emitCall(SymtabEntry *routineId, PascalParser::ArgumentListContext *argListCtx) { 
    cout << "emitCall" << endl << flush;
    // string ret = "";
    // Typespec targetType;
...
int StatementGenerator::createWriteFormat(PascalParser::WriteArgumentsContext *argsCtx, string& format, bool needLF) { 
    cout << "createWriteFormat" << endl << flush;
    int exprCount = 0;
    format += "\"";
    ...
    //         if (fwCtx != nullptr)
    //         {
    //             string sign = ((fwCtx->sign() != nullptr) && (fwCtx->sign()->getText() == "-")) ? "-" : "";
...
void StatementGenerator::emitArgumentsArray(PascalParser::WriteArgumentsContext *argsCtx, int exprCount) { 
    cout <<"emitArgumentsArray" << endl << flush;
...
void StatementGenerator::emitRead(PascalParser::ReadStatementContext *ctx) {
    cout << "emitRead" << endl << flush;
    // emitRead(ctx->readArguments(), false);
}

void StatementGenerator::emitReadln(PascalParser::ReadlnStatementContext *ctx) {
    cout << "emitReadln" << endl << flush;
    // emitRead(ctx->readArguments(), true);
}

void StatementGenerator::emitRead(PascalParser::ReadArgumentsContext *argsCtx, bool needSkip) { 
    cout << "emitRead" << endl << flush;
